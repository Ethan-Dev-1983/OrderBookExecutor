...
namespace OrderBookProcessor.TaskManagement
{
    ...
    internal class LocalStdInTaskWorker:BaseTaskWorker
    {
        ...
        public override async Task ExecuteTask()
        {
            ...
            if (!CheckShell()) {
                 throw new Exception("Pipe mode can only be supported in Bash Shell instead of Powershell. Kindly run OrderBookProcessor <depth_levels> [input_file|input_url] instead in PowerShell");
            }
